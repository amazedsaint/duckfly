...
@MainActor final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate, WKDownloadDelegate, WKScriptMessageHandler {
    ...
    let testing = CommandLine.arguments.contains("--self-test") || CommandLine.arguments.contains("--self-test-room")
    ...
    func startSelfTest() {
        testTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] _ in
            Task { @MainActor in
                ...
                do {
                    let path = Bundle.main.resourceURL!.appendingPathComponent(CommandLine.arguments.contains("--self-test-room") ? "RoomSmoke.js" : "NativeSmoke.js")
