        ...
        try:
            with contextlib.redirect_stdout(sys.stderr):
                if use_bam:
                    ...
                    m,d,ctrl,names=ip.load_mujoco_with_bam(str(scene_path),b,0.005,0.1,ip.BAM_VIN_MIN)
                else:
                    m=mujoco.MjModel.from_xml_path(str(scene_path)); m.opt.timestep=.005; d=mujoco.MjData(m); ctrl=None
            ...
            row['error']=repr(e)
(ROOT/output_name).write_text(json.dumps(report,indent=2)+'\n')
