  ...
  const explain=()=>{
    ...
    if(existing){
      ...
      $('#prop-profile-note').textContent=profile==='existing-physics'
        ...
        : 'This object moves in a straight line. Choose a new behavior or drag it to stop.';
      return;
    }
    ...
    $('#apply-behavior').textContent=restart?'Apply physics & restart':'Apply behavior';
    $('#prop-profile-note').textContent=moving?'The object follows a fixed path. The duck sees the object but not the dotted guide. Drag the object to stop its path.':patch.movable?
      `${restart?'Selected preset':'Current physics'}: ${weightLabel(patch.mass)} · friction ${patch.friction}. The object responds to gravity and collisions.${restart?' Applying this preset restarts the scene.':''}`:
  ...
  return (scene,state,selected)=>{
    ...
    if(nextKey!==key){
      ...
      if(profile.startsWith('existing-')){
        const option=document.createElement('option');option.value=profile;option.textContent=profile==='existing-physics'?'Custom physics':'Straight-line motion';option.dataset.existing='true';$('#prop-profile').append(option);
      }
    ...
    $('#prop-live-state').textContent=state?.paused?'Paused · Run or Step to see the effect.':prop.motion.some(Boolean)&&!prop.behavior?'Applying a preset replaces the current straight-line motion.':'Changes affect the selected object only.';
  };
}
