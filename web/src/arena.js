...
const decode = (s, Type) =>
  new Type(Uint8Array.from(atob(s), (c) => c.charCodeAt(0)).buffer);
export class Arena {
  constructor(host, sceneData) {
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      "Live 3D Microduck simulation. Drag to orbit; scroll to zoom.",
    );
    host.prepend(this.renderer.domElement);
    this.scene = new THREE.Scene();
    this.scene.fog = new THREE.Fog(0xdce4df, 1.8, 5);
    this.camera = new THREE.PerspectiveCamera(42, 1, 0.005, 30);
    ...
    const key = new THREE.DirectionalLight(0xfff6e9, 3.5);
    ...
    const floor = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xcfdad2, roughness: 1 }),
    );
    ...
    for (const [id, m] of Object.entries(sceneData.meshes)) {
      ...
      g.setAttribute(
        "position",
        new THREE.BufferAttribute(decode(m.vertices, Float32Array), 3),
      );
      g.setAttribute(
        "normal",
        new THREE.BufferAttribute(decode(m.normals, Float32Array), 3),
      );
      g.setIndex(new THREE.BufferAttribute(decode(m.indices, Uint32Array), 1));
    ...
    this.robot = sceneData.geometries.map((g) => {
      ...
      const mesh = new THREE.Mesh(meshes[g.mesh], material);
    ...
    this.conversion = new THREE.Quaternion().setFromAxisAngle(
  ...
  home() {
    this.camera.position
      .copy(this.target)
      .add(new THREE.Vector3(0.44, 0.24, 0.46));
  ...
  update(body) {
    body.poses.forEach((p, i) => {
      const mesh = this.robot[i];
      mesh.visible = true;
      mesh.position.set(p[0], p[2], -p[1]);
      mesh.quaternion.set(p[4], p[5], p[6], p[3]).premultiply(this.conversion);
    });
    const next = new THREE.Vector3(body.position[0], 0.12, -body.position[1]);
    if (this.follow) {
    ...
    this.grid.position.x = Math.round(next.x);
    this.grid.position.z = Math.round(next.z);
  }
}
