// DuckFly modifications: injectable per-instance RNG and an explicit neuron
// silencing mask applied before spike emission for causal interventions.
...
export class LIFSim {
  constructor(circuit, spikeBus = null, locomotorCircuit = null, random = Math.random) {
    this.random = random;
    ...
    this.silencedNeurons = new Uint8Array(n);
    ...
    for (let i = 0; i < n; i++) {
      switch (neurons[i].role) {
        case 'other': this.baseline[i] = 0.010 + this.random() * 0.060; break;
  ...
  step(ms) {
    ...
    for (let t = 0; t < ms; t++) {
      ...
      if (this.simMs >= this.burstNext) {
        ...
        this.burstNext = this.simMs + 15000 + Math.floor(this.random() * 25001);
      }
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (this.random() < p) vi += this.noiseKick;
        v[i] = vi;
      }
      if (this.loomL > 0.001) {
      ...
      let nSpiked = 0;
      for (let i = 0; i < n; i++) {
        if (this.silencedNeurons[i]) { v[i] = 0; refr[i] = 0; continue; }
