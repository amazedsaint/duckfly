async host => {
  ...
  const receipts=[],errors=[],report={format:'duckfly-action-recording-scope-browser',version:1,controlledRoomFixture:true,transport:'Controlled delivery through actual ExperimentRoom callbacks; no RTC transport acceptance',independentContexts:true,receipts,errors};
  ...
  try{
    ...
    await host.exposeFunction('__scopeDeliver',async data=>guest.evaluate(data=>{
      const room=window.__scopeRoom;
      if(room?.role==='guest'){room.lastSequence++;room.onState(data);}
    },data));
    for(const page of [host,guest])await page.evaluate(async()=>{
      const {ExperimentRoom}=await import('/src/lab/room.js');
      ExperimentRoom.prototype.sendState=function(data){
        window.__scopeOutgoing=structuredClone(data);
        if(this.role==='host'&&window.__scopeDeliver)window.__scopeDeliver(data);
      };
      ExperimentRoom.prototype.invite=async function(){
        this.disconnect();this.role='host';this.room=crypto.randomUUID();this.connected=true;window.__scopeRoom=this;
        this.onStatus({role:'host',state:'connected',message:'Controlled local fixture host'});
        return JSON.stringify({fixture:true,room:this.room,padding:'.'.repeat(120)});
      };
      ExperimentRoom.prototype.join=async function(offer){
        const data=JSON.parse(offer);if(!data.fixture)throw Error('Expected controlled fixture invitation');
        this.disconnect();this.role='guest';this.room=data.room;this.connected=true;window.__scopeRoom=this;this.lastSequence=-1;
        this.onStatus({role:'guest',state:'connected',message:'Controlled local fixture guest'});
        return JSON.stringify({fixture:true,room:this.room,padding:'.'.repeat(120)});
      };
      ExperimentRoom.prototype.accept=async function(){this.onStatus({role:'host',state:'connected',message:'Controlled local fixture connected'});};
    });
    ...
    stage='controlled room delivery';await pair();
    ...
    report.initialTransport={fixture:true,actualRoomCallbacks:true,actualHostWorkerState:true,peerConnectionUsed:false};
    ...
    await guest.locator('#inspect-event').click();await guest.screenshot({path:'output/playwright/action-scope-fixture-guest.png'});await guest.getByRole('button',{name:'Close action inspector'}).click();
    check(errors.length===0,'Browser page errors: '+JSON.stringify(errors));
    report.passed=true;
  }catch(error){report.passed=false;report.stage=stage;report.failure=error.message;report.transportAtFailure={host:await rtc(host),guest:await rtc(guest)};await host.screenshot({path:'output/playwright/action-scope-fixture-failure-host.png'});await guest.screenshot({path:'output/playwright/action-scope-fixture-failure-guest.png'});}
  finally{await host.evaluate(report=>window.__actionScopeFixtureReport=report,report);await context.close();}
  return report;
}
