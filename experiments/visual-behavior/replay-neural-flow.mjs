...
for (const [file, expected] of Object.entries(summary.sourceHashes)) {
  const actual = file === 'neuralMapFlow' ? hash(readVision('readouts/neural-map-flow.js')) : hash(fs.readFileSync(path.join(here, file)));
...
const selected = process.argv.includes('--all') ? records : records.filter((record, i) => i === 0 || record.spec.seed === 4000);
const checked = [];
try {
...
const report = {
  format: 'duckfly-neural-map-flow-replay', version: 1, passed: true, exactNeuralMapHashes: true, exactDiagnostics: true,
  sourceSha256: hash(fs.readFileSync(fileURLToPath(import.meta.url))), summarySha256: hash(read('summary.json')), checked,
  coverage: process.argv.includes('--all') ? 'all trials' : 'one calibration trial plus every moving/control family at held-out seed 4000',
};
