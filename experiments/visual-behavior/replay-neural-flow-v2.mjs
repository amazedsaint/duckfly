...
const selected = process.argv.includes('--all') ? records : records.filter((record, i) => i === 0 || record.spec.seed === 6000);
const checked = [];
try {
...
const report = {
  ...
  coverage: process.argv.includes('--all') ? 'all trials' : 'one calibration trial plus every moving/control family at held-out seed 6000',
};
