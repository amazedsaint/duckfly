...
export class SparseVisionModel {
  ...
  allocate(array){if(this.disposed)throw Error('Model disposed');const pointer=this.core.allocate(array.length);new Uint8Array(this.core.memory.buffer,pointer,array.byteLength).set(new Uint8Array(array.buffer,array.byteOffset,array.byteLength));this.allocations.push([pointer,array.length]);return pointer;}
  eye(initial=this.arrays.steadyState){
    ...
    },checkpoint:()=>{if(this.disposed||eye.disposed)throw Error('Model disposed');return new Float32Array(this.core.memory.buffer,state,n).slice();},restore:v=>{
      if(this.disposed||eye.disposed)throw Error('Model disposed');
