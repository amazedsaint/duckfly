...
export async function createSpatialFlyvisReadout({ manifest, arrays, wasmBytes }) {
  ...
  const state = activity => {
    if (!(activity instanceof Float32Array) || activity.length !== manifest.nodes || !activity.every(Number.isFinite)) {
  ...
  function baseline(activity, { id, neuralTime, method, conditioningSeconds = 0 }) {
    ...
    // Frozen ordinary arrays make the reusable baseline safe to share and serialize.
    const result = freezeTree({
      id, modelId: metadata.model.id, checkpointSha256: metadata.model.checkpointSha256,
  ...
  return Object.freeze({ metadata, baseline, extract });
}
