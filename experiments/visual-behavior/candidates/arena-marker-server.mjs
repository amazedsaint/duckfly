...
for(const f of['arena-marker-browser.mjs','arena-marker-server.mjs'])sources[f]=sha(await readFile(new URL(f,import.meta.url)));
...
const server=createServer(async(req,res)=>{res.setHeader('Access-Control-Allow-Origin','http://127.0.0.1:5182');res.setHeader('Access-Control-Allow-Headers','Content-Type');res.setHeader('Access-Control-Allow-Methods','GET,POST,OPTIONS');try{
 ...
 if(req.method==='GET'&&req.url==='/arena-marker-browser.mjs'){res.setHeader('Content-Type','text/javascript');res.end(await readFile(new URL('arena-marker-browser.mjs',import.meta.url)));return;}
 ...
 const data=JSON.parse(Buffer.concat(chunks));if(data.probe!=='actual-arena-marker-v1'||data.records.length!==32||data.records.some(r=>r.frames.length!==51))throw Error('Incomplete cohort');
 ...
 await writeFile(new URL('../reports/candidates-arena-marker-v1.json.gz',import.meta.url),bytes);await writeFile(new URL('../reports/candidates-arena-marker-v1-summary.json',import.meta.url),JSON.stringify(summary,null,2)+'\n');res.setHeader('Content-Type','application/json');res.end(JSON.stringify(summary));
