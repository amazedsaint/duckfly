...
export async function runArenaMarkerV2(){
 ...
 for(const family of families)for(let index=0;index<4;index++){
  const scene=defaultScene('target'),p=scene.props[0],distance=[.62,.77,.9,1.08][index],diameter=[.055,.07,.085,.105][index];
  ...
  try{
   for(let i=0;i<=50;i++){
    const time=i*.04,position=[distance,-.17+.17*time,.17];
    world.moveProp(p.id,position,0);
    if(scene.props.some(p=>p.id===distractor.id))world.moveProp(distractor.id,[distance,family==='separated-distractor'?.23:.18-.17*time,family==='separated-distractor'?.33:.17],0);
   ...
   records.push({id:`arena-marker-v2-${family}-${index}`,family,index,scene,frames});
  }finally{world.dispose();}
 }
 const payload={version:1,probe:'actual-arena-marker-v2',createdAt:new Date().toISOString(),interpretation:'Actual rendered target and opaque occluder. Posed objects; no body policy. Object IDs/poses used only for ground-truth labels; inference receives RGBA and capture identity.',records};
