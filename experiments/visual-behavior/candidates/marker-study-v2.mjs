...
import{ClearMarkerTracker,MARKER_SETTINGS}from'./marker-tracker-v2.mjs';
import{markerCases,markerFrame,markerTimes}from'./marker-movies-v2.mjs';
...
for(const f of ['marker-tracker-v2.mjs','marker-movies-v2.mjs','marker-study-v2.mjs','MARKER-PLAN-v2.md'])sources[f]=sha(await readFile(new URL(f,import.meta.url)));
sources['web/src/lab/vision.js']=sha(await readFile(new URL('../../../web/src/lab/vision.js',import.meta.url)));
if(split==='heldout'){const frozen=JSON.parse(await readFile(new URL('candidates-marker-v2-frozen.json',reports),'utf8'));if(JSON.stringify(frozen.sources)!==JSON.stringify(sources))throw Error('Frozen source mismatch');}
...
await writeFile(new URL(`candidates-marker-v2-${split}-movies.rgba.gz`,reports),compressed);await writeFile(new URL(`candidates-marker-v2-${split}-traces.json.gz`,reports),traces);
await writeFile(new URL(`candidates-marker-v2-${split}-summary.json`,reports),JSON.stringify(summary,null,2)+'\n');
if(split==='calibration')await writeFile(new URL('candidates-marker-v2-frozen.json',reports),JSON.stringify({sources,settings:MARKER_SETTINGS},null,2)+'\n');
