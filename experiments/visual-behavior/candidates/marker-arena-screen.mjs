...
import{ClearMarkerTracker}from'./marker-tracker.mjs';import{VisionEncoder}from'../../../web/src/lab/vision.js';
const reports=new URL('../reports/',import.meta.url),sha=b=>createHash('sha256').update(b).digest('hex'),bytes=await readFile(new URL('candidates-arena-marker-v1.json.gz',reports)),data=JSON.parse(gunzipSync(bytes));
const frozen=JSON.parse(await readFile(new URL('candidates-marker-frozen.json',reports),'utf8'));if(sha(await readFile(new URL('marker-tracker.mjs',import.meta.url)))!==frozen.sources['marker-tracker.mjs'])throw Error('Frozen tracker changed');
const records=[];
...
const safe=Object.values(groups).filter(g=>g.eligibleFrames),gates={coverage:safe.every(g=>g.coverage>=.9),precision:safe.every(g=>g.wrongRate!==null&&g.wrongRate<=.01),ambiguityAbstention:records.every(r=>r.confidentAmbiguousFrames===0),noTargetRejection:records.every(r=>r.falseTargetFrames===0)};gates.pass=Object.values(gates).every(Boolean);
const traceBytes=gzipSync(JSON.stringify({records}),{level:9}),summary={sourceArtifactSha256:sha(bytes),trackerSha256:frozen.sources['marker-tracker.mjs'],screenSha256:sha(await readFile(new URL('marker-arena-screen.mjs',import.meta.url))),movies:records.length,groups,gates,traceArtifactSha256:sha(traceBytes),productionPromotion:false,biologicalCircuit:false,bodyUsefulness:false};
await writeFile(new URL('candidates-marker-arena-screen-traces.json.gz',reports),traceBytes);await writeFile(new URL('candidates-marker-arena-screen-summary.json',reports),JSON.stringify(summary,null,2)+'\n');console.log(JSON.stringify(summary,null,2));
