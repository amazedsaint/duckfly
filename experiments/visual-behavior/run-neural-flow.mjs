...
import { neuralFlowPlan, neuralFlowInputs } from './neural-flow-stimuli.mjs';

const here = path.dirname(fileURLToPath(import.meta.url));
const destination = path.resolve(process.argv[2] ?? path.join(here, 'reports/neural-flow-v1'));
...
const sourceFiles = ['run-neural-flow.mjs', 'neural-flow-stimuli.mjs', 'NEURAL-FLOW-PROTOCOL.md', 'load-model.mjs'];
const sourceHashes = Object.fromEntries(sourceFiles.map(file => [file, hash(fs.readFileSync(path.join(here, file)))]));
sourceHashes.neuralMapFlow = hash(readVision('readouts/neural-map-flow.js'));
    ...
    try {
      for (let frameId = 0; frameId < packets.length; frameId++) {
        ...
        if (previous) {
          ...
          if (frameId === 12 && spec.split === 'heldout' && spec.seed === 4000 && ['grating', 'edge-on', 'edge-off'].includes(spec.family)) {
...
const summary = {
  format: 'duckfly-neural-map-flow-study', version: 1, startedAt, finishedAt: new Date().toISOString(), elapsedSeconds: (performance.now() - started) / 1000,
