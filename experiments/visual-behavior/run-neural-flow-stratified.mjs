...
import { neuralFlowPlan, neuralFlowInputs } from './neural-flow-stratified-stimuli.mjs';
import { assertNeuralFlowCoverage } from './neural-flow-coverage.mjs';
...
const plan = neuralFlowPlan(), generatorCoverage = assertNeuralFlowCoverage(plan);
const referenceCalibrationBytes = fs.readFileSync(path.join(here, 'reports/neural-flow-v2/calibration.json'));
const referenceCalibration = JSON.parse(referenceCalibrationBytes);
...
const sourceFiles = ['run-neural-flow-stratified.mjs', 'neural-flow-stratified-stimuli.mjs', 'neural-flow-stimuli.mjs', 'neural-flow-coverage.mjs', 'NEURAL-FLOW-STRATIFIED-PROTOCOL.md', 'NEURAL-FLOW-V2-PROTOCOL.md', 'NEURAL-FLOW-PROTOCOL.md', 'load-model.mjs'];
...
const records = [], inputChunks = [], conditioned = new Map(), intervalCosts = [], extractionCosts = [], decoderCosts = [], fadeCosts = [], ablations = [];
...
try {
  for (const spec of plan) {
    ...
    try {
      for (let frameId = 0; frameId < packets.length; frameId++) {
        ...
        if (previous) {
          ...
          if (frameId === 12 && spec.split === 'heldout' && spec.seed === 8000 && ['grating', 'edge-on', 'edge-off'].includes(spec.family)) {
    ...
    if (records.length === 16) {
      ...
      if (minimumGradient !== referenceCalibration.minimumGradient) throw Error('Frozen v2 calibration changed; stratified confirmation forbids threshold retuning');
...
const summary = {
  ...
  studyId: 'neural-flow-v2-stratified', generatorCoverage,
  referenceCalibration: { file: '../neural-flow-v2/calibration.json', sha256: hash(referenceCalibrationBytes), minimumGradient: referenceCalibration.minimumGradient, retuned: false },
  ...
  gates: { moving: movingResults.every(result => result.passed), confounds: confounds.every(result => result.passed),
    generatorCoverage: generatorCoverage.passed,
