...
for(const family of ['kick','standing','recovery'])for(let seed=0;seed<8;seed++)for(const condition of family==='kick'?['walking','kick']:['walking','standing']){
 ...
 const w=new LabWorld(mj,template,sessions.walking,ort.Tensor,scene,sessions),robot=w.robots[0],trace=[];let initialBall=null;
 try{
  for(let tick=0;tick<400;tick++){
   ...
   const vx=family==='kick'&&tick>=225?.15:0,s=await w.step({'duck-1':{vx,yaw:0,head:[0,0,0,0],policy:robot.session===sessions.kick?'kick':robot.session===sessions.standing?'standing':'walking'}}),b=s.ducks[0];
...
fs.writeFileSync(new URL('reports/skills-rerun.json.gz',import.meta.url),gzipSync(JSON.stringify(report)));fs.writeFileSync(new URL('reports/skills-rerun-summary.json',import.meta.url),JSON.stringify({...report,trials:trials.map(({trace,...t})=>t)},null,2));
