...
import { sourceGuardPlugin, validateSources } from './source-guard.mjs';
const reports=fileURLToPath(new URL('./reports/',import.meta.url));
export default {...base,root:fileURLToPath(new URL('../../web',import.meta.url)),server:{host:'127.0.0.1',port:5190,strictPort:true,watch:null,hmr:false},plugins:[sourceGuardPlugin(),{name:'temporal-evidence',configureServer(server){
 server.middlewares.use('/__temporal',async(req,res)=>{
  try{
   ...
   if(event==='start'){
    const sources=await validateSources(data);
    await writeFile(path,JSON.stringify({event,data})+'\n',{flag:'wx'});
    await writeFile(reports+run+'.sources.json.gz',gzipSync(JSON.stringify(sources)),{flag:'wx'});
   }
