...
export function motionScene(c,gfGain=6){const s=defaultScene('empty');s.seed=c.id;s.name=c.id;s.challenge.duration=3;s.challenge.goal=[1.5,0];
 Object.assign(s.ducks[0],{visionModel:'motion-opponency-v1',mode:c.family==='scan'?'manual':'brain',gfGain});
...
export function configureMotion(e,c,condition){const a=e.agents.get('duck-1');
 ...
 if(condition==='original'||condition==='baseline')return;
