...
export class HazardFeedback {
  ...
  observe({risk, time, source, valid = true}) {
    ...
    if (!valid || !Number.isFinite(risk) || risk < 0 || risk > 1) {
      // Covering an eye can invalidate the latest frame without advancing its clock.
      const advanced = s.captureTime === null || time > s.captureTime;
      if (advanced) s.captureTime = time;
      Object.assign(s, {valid: false, risk: null, lowSince: null, lowThrough: null, pulseUntil: null});
      return advanced;
    }
